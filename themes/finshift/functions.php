 ...
 * Implements NewsArticle, WebSite, and CollectionPage schemas.
 */
function finshift_output_schema_json() {
    $schema = array();
    ...
    if ( is_single() ) {
        global $post;
        $thumb_url = has_post_thumbnail() ? get_the_post_thumbnail_url($post->ID, 'full') : get_template_directory_uri() . '/assets/images/hero-bg.png';
        
        // Get Summary for description if available
        $summary = get_post_meta($post->ID, '_ai_structured_summary', true);
        $description = '';
        if ($summary) {
        ...
        $schema = array(
            ...
            'url' => get_category_link($obj->term_id)
        );
    }

    if ( ! empty( $schema ) ) {
        echo "\n<!-- Schema.org JSON-LD generated by FinShift -->\n";
        echo '<script type="application/ld+json">' . json_encode( $schema, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) . '</script>' . "\n";
    }
}
add_action( 'wp_head', 'finshift_output_schema_json' );
